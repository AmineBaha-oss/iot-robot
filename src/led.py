...
class Led:
    def __init__(self):
        ...
        if self.connect_version == 1 and self.pi_version == 1:
            self.strip = Freenove_RPI_WS281X(self.param.get_param('Led_Count') or 8, self.param.get_param('Led_Brightness') or 255, 'RGB')
            self.is_support_led_function = True

        elif self.connect_version == 2 and (self.pi_version == 1 or self.pi_version == 2):
            self.strip = Freenove_SPI_LedPixel(self.param.get_param('Led_Count') or 8, self.param.get_param('Led_Brightness') or 255, 'GRB')
            self.is_support_led_function = True
